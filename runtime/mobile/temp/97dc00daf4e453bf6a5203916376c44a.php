 ...
 * @LastEditTime: 2021-10-08 17:54:16
 * @Description: Forward, no stop
-->
...
<!--    <li <?php if($leftmenu=='cm'): ?> class="layui-this"<?php endif; ?>><a href="<?php echo url('finance/cmfinance'); ?>"><?php echo lang('mobile_finance.cm_title'); ?></a></li>-->
    ...
    <li <?php if($leftmenu=='yka'): ?> class="layui-this"<?php endif; ?>><a href="<?php echo url('finance/yingkuitongji'); ?>"><?php echo lang('finance.yinkui_tongji'); ?></a></li>
			...
			<div class="mt-10">
				<!--<?php echo lang('tradelog.tab_list_f'); ?>--><?php echo lang('finance.cm_title'); ?>： <span>$ <?php echo htmlentities($sum_usd); ?></span>
			</div>
			<div class="mt-10">
			...
			<table class="layui-table" lay-skin="nob">
				...
				<tr id="list_<?php echo htmlentities(strtolower($vo['title'])); ?>">
					<td><span class="color-blue font-18"><?php echo htmlentities($vo['title']); ?></span>
						...
						<p class="mt-10"><?php echo htmlentities($vo['cm_money']); ?></p>
					</td>
					<td><span>&nbsp;</span>
						<p class="color-l-blue mt-10"><?php echo lang('finance.top_lock'); ?></p>
						<p class="mt-10"><?php echo htmlentities($vo['lock_cm_money']); ?></p>
					</td>
					<td class="text-right">
						...
						<p class="mt-10"><?php echo htmlentities($vo['cm_usd']); ?></p>
					</td>
					</tr>
					<tr class='memu_<?php echo htmlentities(strtolower($vo['title'])); ?> hidebox memu_box' style="border-top:0;">
						<td colspan="3" class="text-center">
							...
							<a href="<?php echo url('finance/transfer',['type'=>5]); ?>" class="layui-btn layui-btn-normal"><?php echo lang('finance.right_transfer'); ?></a>
						</td>
					</tr>
							...
							<div class="log hidebox">
								<table class="layui-table flow-table" lay-skin="nob">
									...
									<tr>
										...
										<th><?php echo lang('finance.flow_all'); ?><br><?php echo lang('finance.flow_rate'); ?></th>
	...
	layui.use(['layer', 'jquery', 'form', 'element', 'flow'], function(){
		...
		window.findlog = function(a,b,id){
			...
			,done: function(page, next){ //到达临界点（默认滚动触发），触发下一页
				var lis = [];
				$.post("<?php echo url('finance/findlog'); ?>",{id:id,page:page,type:3,from:5,to:5,cm:1}, function(res){
					layui.each(res.data, function(index, item){
					lis.push('<tr><td>'+ item.create_time +'<br>'+ item.title +'</td><td>'+ item.all_account +'<br>'+ item.remark +'</td><td>'+ item.type +'<br>'+ item.status +'</td></tr>');
					}); 
