...
class Order extends IndexController
{
    ...
    public function doseconds()
    {
        if(request()->isPost()){
            ...
            $win_or_lose = $m_user->where('id',$indata['uid'])->value('qiquanqs_status');
            $mustWin = $m_user->where('id',$indata['uid'])->value('qiquanqy_status');
            if($win_or_lose == 1){
                //must lose
                $indata['kong_type'] = 2;
            }
            ...
            if($id){
                ...
                try{
                    ...
                    if($order['op_status']==1){
                        ...
                        $win_or_lose = $m_user->where('id',$order['uid'])->value('qiquanqs_status');
                        $mustWin = $m_user->where('id',$order['uid'])->value('qiquanqy_status');
                        $op_order_kong = 50;
