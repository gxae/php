...
class User extends AdminController
{
    ...
    /**
     * @NodeAnotation(title="调整用户输赢")
     */
    public function winlose($id)
    {
        $row = $this->model->find($id);
        empty($row) && $this->error('数据不存在');
        FoxCommon::check_member_wallet($id);
        $post[$this->request->post('field')] = $this->request->post('value');
        $save = $row->save($post);
        $save ? $this->success('保存成功') : $this->error('保存失败');
    }
    ...
    /**
     * @NodeAnotation(title="收款信息")
     */
    public function collection($id)
    {
        $collection = $this->wallet_model->where('product_id', 6)->where('uid', $id)->field('withdraw_erc_address,withdraw_trc_address,yhkdz,yhkkhh,yhkxm')->find();
        
        
        empty($collection) && $this->error('数据不存在');
        ...
        if ($this->request->isAjax()) {
            ...
            try {
                ...
                $save = $this->wallet_model->where('product_id', 6)->where('uid', $id)->update(['withdraw_trc_address' => $post['withdraw_trc_address'],'withdraw_erc_address'=>$post['withdraw_erc_address'],'yhkdz'=>$post['yhkdz'],'yhkkhh'=>$post['yhkkhh'],'yhkxm'=>$post['yhkxm']]);
    ...
    /**
     * @NodeAnotation(title="认证信息")
     */
    public function certification($id)
    {
        $card = $this->card_model->where('uid', $id)->field('name,card')->find();
        empty($card) && $this->error('数据不存在,暂未认证信息');
        if ($this->request->isAjax()) {
            $post = $this->request->post();
            $rule = [];
            $this->validate($post, $rule);
            ...
            try {
                ...
                $save = $this->card_model->where('uid', $id)->update(['name' => $post['name'],'card'=>$post['card']]);
