        ...
        if ($orderlist) {
            foreach ($orderlist as $k => $v) {
                try {
                    ...
                    if ($num_bb > 0 && $num_aa > $num_bb && $order['kong_type'] == 0) {
                        ...
                        if ($odata['is_win'] == 1) {
                            ...
                            $odata['all_fee'] = bc_mul($order['op_number'], $order['play_prop'] / 100);
                            $now_money        = bc_add($user_wallet['op_money'], bc_sub($order['op_number'], $odata['all_fee']));
                            if ($m_order->where('id', $order['id'])->update($odata)) {
                                $m_wallet->where('id', $user_wallet['id'])->update(['op_money' => $now_money]);
                                $lgdata['account']     = bc_sub($order['op_number'], $odata['all_fee']);
                                $lgdata['wallet_id']   = $user_wallet['id'];
                                $lgdata['product_id']  = $user_wallet['product_id'];
                                $lgdata['uid']         = $order['uid'];
                                $lgdata['is_test']     = $is_test;
                                ...
                                $lgdata['after']       = $now_money;
                                $lgdata['account_sxf'] = 0;
                                $lgdata['all_account'] = bc_sub($order['op_number'], $odata['all_fee']);
                                $lgdata['type']        = 6;
                                $lgdata['title']       = $productBase['title'];
                                $lgdata['order_type']  = 2;
                                //赢返
        ...
        if ($orderlist) {
            ...
            foreach ($orderlist as $k => $v) {
                ...
                foreach ($pro_order as $pk => $pv) {
                    ...
                    foreach ($user_order as $uk => $uv) {
                        $order    = $m_order->where('id', $uv['id'])->find();
                        $mustLose = $m_user->where('id', $order['uid'])->value('heyueqs_status');
                        $mustWin  = $m_user->where('id', $order['uid'])->value('heyueqy_status');
                        $chajia   = $order['buy_price'] - $close_price;

                        if ($mustLose == 1) {
                            //must lose
                            //做多
                            if ($order['style'] == 1) {
                                if ($close_price > $order['buy_price']) {
                                    $close_price = $close_price - abs($chajia) - rand(1, 10);
                                }
                            }
                            //做少
                            if ($order['style'] == 2) {
